...
FIXED_SKILL_KEYWORDS = {
    ...
    "r_required": [r"\br language\b", r"\bprogramming in r\b", r"\br\/shiny\b", r"\br studio\b", r"\brstudio\b"],
    ...
    "analytics_required": [r"\banalytics\b", r"\bdata analysis\b", r"\bdata analyst\b", r"\bbusiness intelligence\b", r"\binsights\b"],
...
def standardize_jobs(
    ...
    relevance = df.apply(
        lambda row: classify_relevance(row["job_title"], row["description"]),
        axis=1
    )
    df["relevance_label"] = relevance.apply(lambda x: x[0])
    df["relevance_score"] = relevance.apply(lambda x: x[1])
...
with st.sidebar:
    ...
    skills_input = st.text_input(
        "Skills (manual input)",
        placeholder="e.g. python, sql, excel, analytics, branding, product strategy"
    )
    st.markdown(
        '<div class="small-grey">Type any skills you want. The sheet will still save fixed skill flags.</div>',
        unsafe_allow_html=True
    )

    companies_input = st.text_input(
        "Companies (optional)",
        placeholder="e.g. Grab, Shopee, Unilever, McKinsey, Google"
    )

    location_input = st.text_input(
        "Location *",
        value="Vietnam",
        placeholder="e.g. Vietnam, Ho Chi Minh City, Vietnam, APAC, Remote"
    )

    country_search = st.text_input(
        "Indeed / Glassdoor country",
        value="Vietnam",
        placeholder="e.g. Vietnam, USA, Singapore, Japan"
    )
    st.markdown(
        '<div class="small-grey">Useful for Indeed / Glassdoor country targeting.</div>',
        unsafe_allow_html=True
    )

    work_mode_input = st.selectbox("Work mode", WORK_MODE_OPTIONS, index=1)
    duration_input = st.selectbox("Duration", DURATION_OPTIONS, index=4)

    google_search_term = st.text_input(
        "Google Jobs query (optional but recommended)",
        placeholder="e.g. remote marketing analytics jobs APAC"
    )
    st.markdown(
        '<div class="small-grey">Google Jobs works best with a very specific query.</div>',
        unsafe_allow_html=True
    )
...
if run_search:
    ...
    raw_jobs["relevance_label"] = raw_jobs.apply(
        lambda x: classify_relevance(
            x.get("title", ""),
            x.get("description", "")
        )[0],
        axis=1
    )
    raw_jobs["relevance_score"] = raw_jobs.apply(
        lambda x: classify_relevance(
            x.get("title", ""),
            x.get("description", "")
        )[1],
